...
*/
private["_sessionID","_parameters","_victimNetID","_hostageTaker","_victim","_humanity","_newScore"];
...
try 
{
	...
	_humanity = getNumber (configFile >> "CfgSettings" >> "Humanity" >> "Handcuffs" >> "trapping");
	[_hostageTaker, "handcuffResponse", [str _humanity]] call ExileServer_system_network_send_to;
	_newScore = _hostageTaker getVariable ["ExileHumanity", 0];
	_newScore = _newScore + _humanity;
	_hostageTaker setVariable ["ExileHumanity", _newScore];
	format["setAccountHumanity:%1:%2", _newScore, getPlayerUID _hostageTaker] call ExileServer_system_database_query_fireAndForget;
}
catch 
{
